...
""" PyTorch SegFormer model with Hyperformer++ Adapter."""
...
class LayerNormHyperNet(nn.Module):
    ...
    def __init__(self, input_dim, projected_task_embedding_dim):
        super(LayerNormHyperNet, self).__init__()
        self.task_embedding_dim = projected_task_embedding_dim
        self.weight_generator = linear_layer(self.task_embedding_dim, input_dim)
        self.bias_generator = linear_layer(self.task_embedding_dim, input_dim)
        
    def forward(self, inputs):
        return self.weight_generator(inputs), self.bias_generator(inputs)


class AdapterHyperNet(nn.Module):
    """This module generates the weights for the meta adapter layers."""

    def __init__(self, input_dim, output_dim, projected_task_embedding_dim):
        super(AdapterHyperNet, self).__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
        self.task_embedding_dim = projected_task_embedding_dim
        # Considers weight and bias parameters for generating adapter weights.
        self.weight_generator = linear_layer(self.task_embedding_dim, self.input_dim * self.output_dim)
...
class AdapterLayer(nn.Module):
    """Adapter Layer with unique hypernetworks."""

    def __init__(self, input_dim, reduction_factor):
        super().__init__()
        self.input_dim = input_dim
        self.down_sample_size = self.input_dim // reduction_factor
        self.projected_task_embedding_dim = 64
        
        self.meta_up_sampler = AdapterHyperNet(self.input_dim, self.down_sample_size, self.projected_task_embedding_dim)
        self.meta_down_sampler = AdapterHyperNet(self.down_sample_size, self.input_dim, self.projected_task_embedding_dim)
        self.activation = nn.GELU()
        self.post_layernorm_hypernet = LayerNormHyperNet(self.input_dim, self.projected_task_embedding_dim)
                
    def forward(self, inputs, mapped_task_embedding):
        weight_up, bias_up = self.meta_up_sampler(mapped_task_embedding)
        weight_down, bias_down = self.meta_down_sampler(mapped_task_embedding)
        ln_weight, ln_bias = self.post_layernorm_hypernet(mapped_task_embedding)
        down = F.linear(inputs, weight=weight_down, bias=bias_down)
        middle = self.activation(down)
        outputs = F.linear(middle, weight=weight_up, bias=bias_up)
        outputs = nn.functional.layer_norm(outputs, (self.input_dim,), weight=ln_weight, bias=ln_bias)
...
class TaskHyperNet(nn.Module):
    ...
    def __init__(self, task_embedding_dim, projected_task_embedding_dim):
        super(TaskHyperNet, self).__init__()
        self.task_hidden_dim = 256
        self.projected_task_embedding_dim = projected_task_embedding_dim
        self.task_embeding_generator = nn.Sequential(
            linear_layer(task_embedding_dim, self.task_hidden_dim),
            nn.ReLU(),
...
class SegformerLayer(nn.Module):
    ...
    def forward(self, hidden_states, height, width, task_embedding, adapter, task_num, output_attentions=False): #
        ...
        mlp_output = adapter(mlp_output, task_embedding)
...
class SegformerEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.task_embedding_dim = 512
        self.projected_task_embedding_dim = 64
        # self.task_hypernet = TaskHyperNet(self.task_embedding_dim, self.projected_task_embedding_dim)

        self.task_block_hypernet = TaskHyperNet(self.task_embedding_dim * 2, self.projected_task_embedding_dim)
        
        self.task_embeddings = []
        for _ in range(self.num_tasks):
            self.task_embeddings.append(nn.parameter.Parameter(data=torch.zeros(self.task_embedding_dim), requires_grad=True))#.to('cuda'))
            self.task_embeddings[-1].data.normal_()
        self.task_embeddings = nn.ParameterList(self.task_embeddings)

        self.layer_id_embeddings = []
        for _ in range(total_sf_layers):
            self.layer_id_embeddings.append(nn.parameter.Parameter(data=torch.zeros(self.task_embedding_dim), requires_grad=True))#.to('cuda'))
            self.layer_id_embeddings[-1].data.normal_()
        self.layer_id_embeddings = nn.ParameterList(self.layer_id_embeddings)

        self.reduction_factor = 8
        self.adapters = []
        for i in range(config.num_encoder_blocks):
            self.adapters.append(AdapterLayer(config.hidden_sizes[i], self.reduction_factor))
    ...
    def get_embedding(self, task_embedding, layer_id):
        ...
        embeddings = self.task_block_hypernet(embeddings.view(-1))
        return embeddings
    ...
    def forward(
        ...
        # mapped_task_embedding = self.task_hypernet(self.task_embeddings[task_num])

        t_emb = self.task_embeddings[task_num]
        ...
        layer_id = 0
        for idx, x in enumerate(zip(self.patch_embeddings, self.block, self.adapters, self.layer_norm)):
            embedding_layer, block_layer, adapter, norm_layer = x
            # first, obtain patch embeddings
            hidden_states, height, width = embedding_layer(hidden_states)
            # second, send embeddings through blocks
            for i, blk in enumerate(block_layer):
                mapped_embedding = self.get_embedding(t_emb, layer_id)
                layer_id += 1

                layer_outputs = blk(hidden_states, height, width, mapped_embedding, adapter, task_num, output_attentions)

                # layer_outputs = blk(hidden_states, height, width, mapped_task_embedding, task_num, output_attentions)
